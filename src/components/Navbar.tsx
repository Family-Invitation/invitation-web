...
const Navbar = () => {
  ...
  const MenuNavbarOpen = () => {
    return (
      <div className="fixed inset-x-0 top-0 md:hidden flex flex-col bg-pink-100 h-screen z-40 w-full justify-center items-center gap-5 overflow-hidden">
        <button
  ...
  return (
    <nav>
      {isMobileMenuOpen ? (
        ...
        <div
          className={`fixed inset-x-0 top-0  flex items-center justify-between p-4  md:px-12  transition-all duration-300 z-50  ${
            ...
            <button className="md:hidden text-white" onClick={toggleMobileMenu}>
              <FaBars
                className={`w-6 h-6 transition-all duration-300 transform hover:scale-110 ${
                  scrolling ? "text-pink-500" : "text-white"
                }`}
              />
            </button>
          </div>
